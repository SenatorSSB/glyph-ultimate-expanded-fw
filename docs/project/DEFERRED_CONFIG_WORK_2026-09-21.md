...
The supervisor's isolated checkout `/private/tmp/glyph-execution-20260921`
retains uncommitted partial work atop the canonical base above. This deferral
checkout neither changes nor executes it. The preserved working state includes
staged decoder fixture files (`config.pb.c`, `pb_common.c/h`, `pb_decode.c/h`,
provenance/README), `tools/glyph_config_decode_fixture.py`, and unfinished
checker/harness files `tools/check_glyph_button_mask_characterization.py` and
`tools/fixtures/config_decode_host/button_mask_harness.cpp`. Dirty queue and
...
- Reconcile the retained partial source with live canonical and verify its
  ...
  declarations. Preserve upstream bytes and narrow attributes under reviewed
  guards; do not weaken broader validation.
- Complete the exact-production helper and decoder harness and caller matrix
  across configured, default, persistence, menu and SetConfig sources. Separate
  decoded storage facts, enum/shift diagnostics and physical unknowns.
- Complete every original clean-checkout, positive/negative, source-census and
  validation gate, then obtain fresh independent review and normal publication.
  Partial local results are not a completed characterization or firmware proof.
- Resolve any source-unsettled reject/no-binding policy with the owner through
  a separate future Curator order; no behavior choice is made by this TODO.
...
physical symptom or firmware repair is asserted.
...
GP-VAL-026 remains DONE. GP-CONFIG-011 remains completed historical
