  ...
  "items": [
    {
      ...
      "authorization_snapshot_provenance": "Independent contract Curator /root/contract_curator second source audit against live canonical 1a4b9311c8f7ae6d7cbf0a8680cd976499112f03 and nonauthoritative failed candidate 95efad7 on 2026-09-06. Root supplied exact 27/30 failed aggregate with canonical MATCH. Audit covered every current manifest command and source-consumed Git identity path, not only reported symptoms. Publish this rebound from fresh live canonical before resumed implementation; final repaired implementation requires a separate reviewer.",
      "automated_validation": [
        ...
        "Pass focused adversarial, checker-context, isolated current X1, census, manifest, health, syntax/compile, full 30-current-check aggregate within production budgets, framework, sequence, navigation, agent-surface, exact diff, and a fresh independent postimplementation reviewer before publication.",
        ...
        "Test fixed observation/nuker historical consumers from isolated clone, preserving object availability and existing reachability semantics. Run all 30 current checks on the exact committed candidate, with missing refs/objects negatives and independent full review. Any further substantive topology/semantic gap returns REPAIR_REQUIRED; no third scope expansion in this recovery.",
