...
`glyph-portfolio-20260823-1450` records `GP-HW-001` as Done, while `GP-CTL-001`
and `GP-VAL-002` remain Ready; `GP-SRC-003` is Done after its host-tool guardrail
...
work is recorded. Effective authorized runway is two against target four, so liveness is
