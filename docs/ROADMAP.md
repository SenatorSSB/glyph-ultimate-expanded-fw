...
`configurator`. It does not implement runtime-loaded config, storage,
WebSerial/device write, backend/config.pb write path, firmware flashing
automation, or nunchuk validation.

The required realization pivot is away from RAM-backed active table pointer
publication. RAM-backed active runtime table storage appears unsafe as an
active publication target under this diagnostic, even when
