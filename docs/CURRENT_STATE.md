...
- `tools/install_generated_source_owned_runtime_config.py` can preview or write `GeneratedRuntimeConfigBaseline.current.hpp`; that header is active compile-time table-content source through `UltimateIdentityRuntimeTables.hpp`, so writing it is active firmware source mutation even though the source-owned `RuntimeConfigView` publication path remains unchanged. `GP-SRC-001` is Ready to correct stale inert markers and fail-close legacy writes; `--dry-run` remains non-mutating.
