...
- The candidate-generation diff diagnosis for `docs/runtime_config/fixtures/generated_source_owned_layout_spec.json` is currently classified `TABLE_CONTENT_EQUIVALENT` on this branch: the checked-in inert baseline artifact now matches the generated candidate output and all 28 tables are source-aligned.
...
- Candidate-generation diff diagnosis is documented and executable; the current
  classification is `TABLE_CONTENT_EQUIVALENT`, reflecting the materialized
  inert alias-path branch rather than a drift-only diagnostic.
