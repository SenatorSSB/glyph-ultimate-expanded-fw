...
Status label: CURRENT. <!-- current-runway:start -->{"ready_ids":[],"immediate_ready":0,"recorded_preauthorized":0,"mechanically_activatable_preauthorized":0,"invalidated_preauthorized":0,"hardware_pending":1,"effective_authorized_runway":0,"target_effective_authorized_runway":4,"primary_liveness":"PLANNING_REQUIRED","global_evidence_wait_supported":false}
<!-- current-runway:end -->
...
`docs/archive/README.md` and `docs/calibration/INDEX.md`; prior RUNWAY_OK is historical and the marker above is authoritative.
...
**Current Agentic Operating State:** GP-X1-002 is DONE through reviewed exact-candidate integration 3b4a7c950e1b14e24a88c951c19313f6134f4ea3 and strict completion correspondence. Exact candidate f657715b26d26587a931074ce7dd12c698785290 and original tested UF2 00dc75b65a080830131c3c260558d4daeff45bb528e43d196b8ad94fcd06f451 remain the hardware authority; the integration rebuild was not physically tested. GP-VAL-027 remains DONE and its finite correspondence model keeps the active generated baseline CRITICAL. GP-CONFIG-010 remains LOCAL_ACCEPTANCE_PENDING / INCONCLUSIVE after the consumed 13-Ultimate recovery run: all 13 activation rows, including indices 10 through 12, passed, but the first final reconnect froze at the initial logo and the required post-reconnect 13-profile visibility plus LT4-to-Ult13 observation was not completed. Exact owner Config restoration and accepted GP-X1-002 rollback passed. One bounded reconnect-completion retest is now authorized under GP_CONFIG_010_RECONNECT_COMPLETION_V1 with the complete backup/write/restore envelope; the prior INCONCLUSIVE record and 13/13 activation PASS evidence remain unchanged admissible provenance. GP-CONFIG-012, GP-CONFIG-013 and GP-CONFIG-014 remain REVIEW / OWNER_DEFERRED / NONEXECUTABLE; GP-VAL-011 remains deferred and GP-HW-002 remains research gated. Nunchuk remains NOT_TESTED and root cause remains unproven. The prior inconclusive persistence event remains separate. CUSTOM_RUNNER_NOT_REQUIRED; retired official-configurator historical evidence remains at tools/check_glyph_official_configurator_validation.py; the older import/export compatibility chain is historical-only; and external-remapper evidence remains quarantined.
...
- The project owner reported all expected sole/non-mode X1 offset-41 outputs and no disconnects for the exact pair above, then confirmed the restored prior firmware worked; `docs/calibration/fixtures/x1_offset41_hardware_evidence_2026-09-02.json` records the accepted exact-snapshot evidence.
...
- Overlay/preserve generation is checker-enforced; unowned tables preserve the current source-owned baseline and production example provenance is rejected. The accepted production-authorized X1 overlay owns exactly `kX1Table` at the hardware-validated offset-41 values and preserves all other 27 active table contents.
- Generator modes remain complete offline tooling with explicit modes, 28-row manifests, deterministic digests, stable exits, gates, and fixtures. The accepted X1 intake is now a `NO_OP` against current source after tested-candidate integration; future X1 values remain separately gated. The 27-table literal-body contract is SUPERSEDED.
...
- Safe offline source-owned layout-spec path: the source-owned layout-spec bridge at `tools/convert_coordinate_native_profile_to_source_owned_spec.py` validates generic coordinate-native profiles but fails closed until explicit profile-to-table source authority exists; the direct `tools/generate_source_owned_runtime_config.py --emit-from-layout-spec` path validates the inert layout-spec packet. Neither path loads into firmware.
