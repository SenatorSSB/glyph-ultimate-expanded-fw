...
- `tools/install_generated_source_owned_runtime_config.py` can preview or write `GeneratedRuntimeConfigBaseline.current.hpp` through `--from-layout-spec` or `--from-generated-output`; that header is active compile-time table-content source through `UltimateIdentityRuntimeTables.hpp`, so writing it is active firmware source mutation even though the source-owned `RuntimeConfigView` publication path remains unchanged. `GP-SRC-001` is Ready to correct stale inert markers and fail-close legacy writes; `--dry-run` remains non-mutating.
- `tools/prepare_source_owned_candidate_branch.py` is non-mutating by default, but explicit `--write-source` can write that active table-content header from the older layout-spec route on a clean non-`configurator` branch; it is not current production authority and is inside `GP-SRC-001` guardrail scope.
