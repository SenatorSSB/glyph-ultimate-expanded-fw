# Phase 7A Diagnostic D5A-N1 Hardware Plan
...
This plan records pre-hardware intent and required checks for D5A-N1.
...
- Remove the separate parse-status source-owned alias and route
  `kSourceOwnedCurrentBaselineRuntimeConfig` directly after parse-status gate.
- Keep parse-status gate.
- Keep fallback to `kKnownGoodRuntimeConfig`.
...
| SOURCE-OWNED-ROUTING-001 | runtime_routing | Analog runtime output lookup is routed through direct source-owned view after parse gate | NOT_TESTED |
...
| NO-WRITE-001 | webserial_or_write | No WebSerial/device write not added | NOT_TESTED |
