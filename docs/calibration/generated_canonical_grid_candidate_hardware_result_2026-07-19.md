...
preservation_branch: `docs-runtime-config-canonical-grid-failure-preservation`
...
- `preservation_branch_carries_failed_source`: `false`
...
- The result branch descends from failed active source and is evidence only.
- The preservation branch starts from `origin/configurator` and records this
  evidence without carrying the failed generated artifact source.
...
- The evidence result branch must not be merged into `configurator` because it
