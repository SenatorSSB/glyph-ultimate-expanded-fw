...
ALLOWED_PATH_RE = re.compile(
    r"^(?:docs/runtime_config/|docs/CURRENT_STATE\.md|docs/ROADMAP\.md|tools/|"
...
def validate_branch() -> str:
    ...
    if branch not in {EXPECTED_BRANCH, DOWNSTREAM_ACTIVE_DIAGNOSTIC_BRANCH, MERGED_BRANCH}:
        fail(
...
def parse_source_baseline_order(text: str) -> list[str]:
    ...
    if len(symbols) != EXPECTED_TABLE_COUNT:
        fail(f"source baseline must contain {EXPECTED_TABLE_COUNT} tables, found {len(symbols)}")
...
def parse_generated_artifact(text: str) -> list[tuple[str, list[tuple[int, int]]]]:
...
def validate_equivalence() -> None:
    ...
    generated = parse_generated_artifact(read_required(ARTIFACT))
