...
ARCHIVED_EVIDENCE_ALLOWED_EXACT_CHANGED_PATHS = {
    "docs/CURRENT_STATE.md",
    "docs/ROADMAP.md",
...
RESULT_BRANCH_ALLOWED_EXACT_CHANGED_PATHS = ARCHIVED_EVIDENCE_ALLOWED_EXACT_CHANGED_PATHS
...
def validate_changed_paths(paths: set[str]) -> None:
    branch = current_branch()
    if branch in {RESULT_BRANCH, EVIDENCE_BRANCH}:
        for path in sorted(paths):
            if FORBIDDEN_SOURCE_CHANGED_RE.search(path):
                fail(f"source path changed on archived evidence branch: {path}")
            if FORBIDDEN_CHANGED_RE.search(path):
                fail(f"unsupported storage/write/WebSerial/flashing path changed on archived evidence branch: {path}")
            if path not in ARCHIVED_EVIDENCE_ALLOWED_EXACT_CHANGED_PATHS:
                fail(f"archived evidence branch may change only docs/checker paths, got: {path}")
        return
    for path in sorted(paths):
        if FORBIDDEN_CHANGED_RE.search(path):
...
def validate_failure_fixture(payload: dict[str, Any], failure_text: str) -> None:
    ...
    expected_conclusions = [
        ...
        "future realization should avoid RuntimeConfigView replacement as active customization mechanism unless a narrower hardware-validated diagnostic proves otherwise",
    ...
    for phrase in (
        ...
        "Source-owned active-state preselection remains the last known passing active-runtime boundary.",
        "future realization should avoid `RuntimeConfigView` replacement as the active customization mechanism unless a narrower hardware-validated diagnostic proves otherwise",
...
def validate_failure_result_docs() -> None:
    ...
    for text, label in (
        ...
        require_phrase(text, "source-owned active-state preselection remains", label)
