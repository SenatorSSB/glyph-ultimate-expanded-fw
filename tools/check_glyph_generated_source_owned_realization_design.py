...
ALLOWED_CHECKER_PATHS = {CHECKER_REL, ACTIVE_STORAGE_CHECKER_REL}
...
INERT_SOURCE_SCAFFOLD_RE = re.compile(
    r"^(?:src|include)/runtime_config/generated_source_owned/[A-Za-z0-9_./-]+\.(?:h|hpp|hh|cc|cpp)$"
)
...
def validate_inert_source_scaffold(path: str, fixture: dict[str, Any]) -> None:
...
def validate_changed_paths(paths: set[str], fixture: dict[str, Any]) -> None:
    for path in sorted(paths):
        ...
        if SOURCE_PATH_RE.match(path):
            validate_inert_source_scaffold(path, fixture)
            continue
