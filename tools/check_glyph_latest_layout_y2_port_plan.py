...
IMPLEMENTATION_ALLOWED_EXACT_CHANGED_PATHS = ALLOWED_EXACT_CHANGED_PATHS | {
...
def validate_changed_paths(paths: set[str], branch: str) -> None:
    exact_allowed = ALLOWED_EXACT_CHANGED_PATHS
    prefix_allowed = ALLOWED_PREFIXES
    if branch == IMPLEMENTATION_BRANCH:
        exact_allowed = IMPLEMENTATION_ALLOWED_EXACT_CHANGED_PATHS
        prefix_allowed = IMPLEMENTATION_ALLOWED_PREFIXES
    for path in sorted(paths):
        if branch != IMPLEMENTATION_BRANCH and FORBIDDEN_SOURCE_PATH_RE.search(path):
        ...
        if path in ALLOWED_EXISTING_SOURCE_OWNED_TABLE_CHECKERS:
            continue
        if any(path.startswith(prefix) for prefix in prefix_allowed):
            continue
