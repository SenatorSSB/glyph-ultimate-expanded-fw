...
REFERENCE_KEYS = {"build_flags", "build_src_filter", "lib_deps", "lib_ignore", "custom_nanopb_options"}
REFERENCE_SOURCES = {
    ("platformio.ini", "arduino_pico_base", "build_flags"),
    ("platformio.ini", "arduino_pico_base", "custom_nanopb_options"),
...
def source_records(path: str) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    declaration_orders: dict[tuple[str, str], int] = {}
    reference_orders: dict[tuple[str, str], int] = {}
    ...
    for line in (ROOT / path).read_text(encoding="utf-8").splitlines():
        ...
        if match:
            ...
            if key in SCALARS or key == "extends" or (key in LISTS and value.strip() and not value.strip().startswith("${")):
                order_key = (section, key)
                declaration_orders[order_key] = declaration_orders.get(order_key, 0) + 1
                declarations.append({"path": path, "section": section, "key": key, "raw": line.strip(), "order": SCALAR_ORDERS.get(order_key, declaration_orders[order_key]), "origin": section, "value": value.strip()})
            continue
        if active and line.strip() and line.startswith((" ", "\t")):
            ...
            if token.startswith("${") and token.endswith("}") and (path, section_name, key) in REFERENCE_SOURCES:
                reference_orders[order_key] = reference_orders.get(order_key, 0) + 1
                references.append({"path": path, "section": section_name, "key": key, "raw": line.rstrip(), "order": reference_orders[order_key], "origin": section_name, "expands": token[2:-1]})
            elif (key in LISTS and not token.startswith("${")) or token in NANOPB:
                declaration_orders[order_key] = declaration_orders.get(order_key, 0) + 1
                order = declaration_orders[order_key]
...
def normalize_records(records: list[dict[str, str]]) -> list[dict[str, object]]:
    return [{key: (int(record[key]) if key == "order" else record[key]) for key in ("path", "section", "key", "raw", "order", "origin", "value")} for record in records]
...
def validate(data: dict[str, object]) -> None:
    ...
    if normalize_records(data["declarations"]) != parsed or data["references"] != parsed_refs or data["chain_references"] != parsed_chain:
...
def adversarial_checks(data: dict[str, object]) -> None:
    mutations = [
        ("declarations", lambda items: items[0].update({"order": items[0]["order"] + 1})),
        ("declarations", lambda items: items[0].update({"section": "glyph_base"})),
        ("declarations", lambda items: items[0].update({"value": "invented"})),
        ("references", lambda items: items[0].update({"key": "lib_deps"})),
        ("references", lambda items: items[0].update({"section": "env"})),
        ("source_files", lambda items: items.pop()),
    ]
    for mutation, edit in mutations:
        altered = copy.deepcopy(data)
        edit(altered[mutation])
        try:
            validate(altered)
