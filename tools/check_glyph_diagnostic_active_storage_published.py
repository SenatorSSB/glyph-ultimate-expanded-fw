#!/usr/bin/env python3
"""Validate the dedicated active-storage publication diagnostic branch."""
...
def validate_merged_hardware_result_if_needed(branch: str) -> None:
    if branch != MERGED_BRANCH:
        return
    result_text = read_required(HARDWARE_RESULT_PATH)
    result_fixture = load_json_object(HARDWARE_RESULT_FIXTURE_PATH)
    require_phrase(result_text, "HARDWARE_PASS", "merged hardware result")
    if result_fixture.get("overall_result") != "HARDWARE_PASS":
        fail("merged configurator must preserve diagnostic HARDWARE_PASS result")
    if result_fixture.get("nunchuk_status") != "NOT_TESTED":
        fail("merged diagnostic hardware result must keep nunchuk NOT_TESTED unless validated")
